...
char nextword[80];
int keepalive, paged = 0, decobject = 0, filtermine = 0, on = 1;
SOCKET tel;
HOSTENT *hp;
...
tel = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
if (tel == INVALID_SOCKET)
{
             printf("Client: socket() - Error at socket(): %ld\n", WSAGetLastError());
             WSACleanup();
             exit(1);
             }
setsockopt(tel, IPPROTO_TCP, TCP_NODELAY, (const char *)&on, sizeof(int));             
...
memmove(&clientService.sin_addr, hp->h_addr, hp->h_length);                  
clientService.sin_family = AF_INET;
clientService.sin_port = htons(atoi(port));

/* Connect to Socket */

if (connect(tel, (SOCKADDR*)&clientService, sizeof(clientService)) == SOCKET_ERROR)
{
...
if (match(myin, "help", 1) > 0)
{
                ...
                {
                                   sprintf (sot, "%s view file <filename> \45\45[@emit\45\45]\45\45[@pemit\45\45]", botname);
                                   sendoutput (speaker, sot);
                                   sprintf (sot, "%s read text <filename> \45\45[line\45\45]", botname);
                                   sendoutput (speaker, sot);
...
if (match(myin, "read text", 1) > 0)
{
		...
		if (match(myin, "line", 1) > 0)
		{
			...
			while ((fgets (&linein[0], LINEBUF, rinfile)) != NULL)
			{
				...
				ln = ln + 1;
                        	sprintf (sot, "&LINE%i %s=%s", ln, nextword, linein);
				botput (sot);
				}
			sprintf (sot, "&linenum %s=%i", nextword, ln);
			botput (sot);
			sprintf (sot, "@desc %s=\45[repeat(-,79)\45]", nextword);
			botput (sot);
			sprintf (sot, "@adesc %s=@dolist lnum(v(linenum))=@pemit \45\45#=\45[v(LINE\45[##\45])\45]", nextword);
			botput (sot);
